...
@section('content')
    <div class="container-store py-8 lg:py-12">
        ...
        <p class="icon-label mb-8 text-sm text-gray-600">
        ...
        <div class="grid gap-8 lg:grid-cols-3">
            <div class="lg:col-span-2">
                <form action="{{ route('checkout.store') }}" method="POST" class="space-y-6 rounded-xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
                    @csrf
                    ...
                    <div>
                        ...
                        <input type="tel" id="phone" name="phone" value="{{ old('phone') }}" required class="mt-1 w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-brand-red focus:outline-none focus:ring-1 focus:ring-brand-red">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="address" name="address" required rows="3" class="mt-1 w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-brand-red focus:outline-none focus:ring-1 focus:ring-brand-red">{{ old('address') }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="city" name="city" value="{{ old('city') }}" class="mt-1 w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-brand-red focus:outline-none focus:ring-1 focus:ring-brand-red">
                    </div>

                    <div>
                        <label for="notes" class="block text-sm font-medium text-gray-700">Notes <span class="text-gray-400">(optional)</span></label>
                        <textarea id="notes" name="notes" rows="2" class="mt-1 w-full rounded-md border border-gray-300 px-4 py-2.5 text-sm focus:border-brand-red focus:outline-none focus:ring-1 focus:ring-brand-red">{{ old('notes') }}</textarea>
                    </div>

                    <div class="icon-label rounded-md border border-green-200 bg-green-50 p-4 text-sm text-green-800">
                    ...
                    <button type="submit" class="btn-primary icon-label w-full sm:w-auto">
